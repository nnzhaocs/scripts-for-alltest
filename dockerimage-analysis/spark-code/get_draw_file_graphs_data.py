...
def save_by_type(spark, sc):
    """save by repeat cnt"""
    file_info = spark.read.parquet(uniq_1_copy_file_info)#unique_file_basic_info).filter(F.size('type') > 0)
    ...
    type1 = file_info_df.groupby('type').agg(F.size(F.collect_list('sha256')).alias('total_cnt'), F.sum('avg').alias('sum_size'))
                                            #F.sum('total_sum').alias('sum_size'),
                                            #F.size(F.collect_list('cnt')).alias('uniq_cnt'))
    type1.printSchema()

    type1 = type1.withColumn('mean_size', F.col('sum_size')*1.0/F.col('total_cnt'))
    ...
    #sort_type_size.show()
    sort_type_size.coalesce(400).write.csv(uniq_1_copy_ftype_by_cap)
    #'/redundant_image_analysis/draw_type1_by_cap_med.csv')#draw_type_by_size)
    """
