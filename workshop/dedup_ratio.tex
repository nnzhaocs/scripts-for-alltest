...
in the Dockerfile~\VT{~\cite{Dockerfile}}
...
%
\VT{Exaplain wha tis cowsay?}
%
\VT{We need to know more about those layers. ``related to'' is too vague.}\NZ{addressed}
...
Figure~\ref{fig:dedup-ratio-growth} shows the deduplication ratio growth with layer dataset size. x-axis shows 4 random samples of given sizes from our whole dataset. We see that deduplication ratio increases with layer dataset size.
%
...
% 
As the number of images stored in Docker registry
is increasing dramatically, file-level deduplication can save significant storage space.
