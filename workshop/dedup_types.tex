...
The overall deduplication ratio is \textbf{6.99$\times$}, and most of the type group have a
comparable ratio. 
%
...
%
Source codes,
scripts, and documents have the highest deduplication ratio (\textbf{31.25$\times$} for source
codes, \textbf{50$\times$} for scripts, and \textbf{12.5$\times$} for documents), \textit{which means that
...
replicated C/C++ source code called Google Test~\cite{googletest} which is a
cross-platform C++ test framework and available in GitHub~\cite{github}.
...
%group.
% 
Database related files have the lowest deduplication ratio (\textbf{4.2$\times$}), which
contributes little to the overall savings.

\textit{Finding 2: All the common file types have a high deduplication ratio.
