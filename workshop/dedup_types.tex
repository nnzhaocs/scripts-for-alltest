...
%
Consequently, the deduplication ratio of EOL files is \textbf{7.14$\times$}.
\DIM{We use the dedup ration before and after, maybe we should add it for EOL as well? 1/(1-0.86) = 7.14}
\NZ{addressed}
%
...
\DIM{Okay - but could we say something more precise, e.g., what \% of 77\% is taken by gtest?}
%
Interestingly, we found there are multiple Docker repositories \DIM{Docker repositories?}\NZ{yes} related to Google
...
\textit{Finding 2 \DIM{Where is Finding 1?} \NZ{in sec 4.1: file-level dedup section}: 
	All common file types have a high deduplication ratio.
...
own. \DIM{Ffrom the data we see there is high source code dedup, but this might be too much of a generic statement based on this data.}}
