...
%
We identify \VT{~133} file types (e.g., JPG, C/C++ code, Java files)
using the file type library~\VT{\cite{pymagic}} and then group file types into 7
classes by their usage scenarious:
%
1)~executables, libraries, and object files (EOL)(e.g., .o)
%
...
6)~images (e.g., PNG),
%
7)~database files (e.g., .sqlite). 
%
\VT{I'd add extensions example in parenthesis for every class. E.g., document is not intuitive.}\NZ{addressed}
These classes consitute 88.23\% of the whole dataset.
%
\VT{Weird, ``Oths'' is ~30TB in the graph, so much more than 1.6\%. What's
going on?}\NZ{addressed}
...
image to their Docker repositories, we believe that 
more developers would replicate
%more 
open source codes
%may be replicated 
...
Google Test example)}\NZ{addressed}
%
To
understand why there are so many redundant archival
files, we manually inspected redundant archival files
and found that most of archival files packages open
source codes. For example, we found a redundant zip
files called android-ndk-r12b.zip~\cite{NDK}. It packages the open
source code–Android Native Development Kit (NDK)–
that allows Android application developers to include
native code in their Android application packages, compiled as JNI shared libraries~\cite{NDK}. android-ndk-r12b.zip is
available for downloading from GitHub.
%
