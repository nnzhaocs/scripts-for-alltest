...
We now propose additional optimizations that can help to speed \sysname\ up:
...
%
According to our statistics, only 10\% of the images were pulled
from the Docker Hub more than 360 times from the time when the image is first pulled to Docker Hub
until May 30, 2017. Moreover, we calculated that 90\% of pulls
...
but less effective compression methods than gzip~\cite{lz4}.
%
%\VT{cite a few}
...
%
Thus, file-level dedupilication can be triggered
when the load is low to prevent interference with 
client pulls and pushes.
%
