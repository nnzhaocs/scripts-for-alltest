...
and redundant copies are removed. \alicomment{How are we saving the location
of each file in the layer? It is not clear from the following sentences.}
\NZ{addressed} For each
...
\textit{layer-to-file table}. The \textit{layer-to-file table} also records the 
file path within each layer associated with each file.      
\alicomment{Separately mention pull layer requests}

For all the \emph{pull image} requests, Docker registry will first fetch 
