...
\nancomment{1. add layer-level content addressable
	2. shorten other sections}

Container-based virtualization (such as Linux Containers~(LXC)~\cite{LXC}) has
emerged as a lightweight virtualization alternative.
%
...
Docker~\cite{docker} is a new, popular container-based virtualization
technology that extends LXC with higher level APIs and additional
functionality.
%
It automates the deployment of applications inside Linux containers, and
provides the capability to package an application with its runtime dependencies
into a container~\cite{slacker}.
...
%
A layer contains a subset of the files in the image and usually represents a
...
\paragraph{Layer level content addressable}
%
%\vcomment{Somewhere we need to say that clients talk to registries using REST API.}

%Docker registry stores and exchanges layers as compressed archival files and images
...
%Xen~\cite{xen}) have been extensively used by major cloud
...
%required and system calls go directly to the kernel~\cite{slacker}. 
%%
...
%%LXC, cgroups, and the Linux kernel itself. Since its based on LXC, a Docker
%%container does not include a separate operating system; instead it relies on the
%%operating systems own functionality as provided by the underlying infrastructure.
...
%%supporting an image model where the images configuration can be hashed to generate
%%an ID for the image.
%
