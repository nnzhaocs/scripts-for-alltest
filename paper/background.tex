...
%platforms such as Amazon EC2 as they provides strong isolation guarantees.
%%
...
operating system level. Containers share the same kernel which significantly reduces
overhead as less storage and memory is required and improves startup
times~\cite{ContainerVirtualization}. 

\subsection{Docker}

Docker~\cite{docker} is a new popular container-based virtualization technology that extends LXC
...
This modular design allows layers to be shared between two containers if both
...
sharing.
%
Docker supports multiple storage drivers, \eg, Aufs or Btrfs, which
