...
\vcomment{I find it much more logical to first talk about generic dataset
properties (file sizes, types, layer sizes, irrespective to deduplication) and
only then get into deduplication properties.
%
This subsection (generic properties) can be short, if we don't have much
materials, but it should anyway go before dedup analysis.
%
This comment is both about the paper and the intro.}
%
\vcomment{In general, I think it seems we should have the followin
sections in order (and build the intro the same way): 1) generic
properties, 2) dedup properties, 3) simulation-based analysis of
dedup-capable registry.}
