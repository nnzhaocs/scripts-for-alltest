...
Recently, \emph{containers} %~\cite{process-containers-linux} ~\cite{rosenblum2005virtual} 
have
gained significant traction as an alternative to the extant virtual
machines for virtualization both on
...
namespaces) and resources (e.g., via control
groups)~\cite{slacker}.  This enables containers to use
...
analysisdockergithub}. However, the registry and its
...
to 63~MB. 
This results in a compression ratio of \gap and shows the potential efffectiveness of compression\nancomment{add compression ratio}.
