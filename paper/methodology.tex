...
We used Docker registry client~\vcomment{insert the name, \nancomment{addressed}} library~\cite{dockerregistryclient} to simultaneously
...
the metadata information for each subdirectory. Note that we excluded the compressed layer
...
%
A complete layer profile consists of:
...
directory count;
file count;
layer directory depth \lrcomment{How is this different from directory depth below?};
\}

\item Compression ratios: \{
ALS-to-CLS;
FSL-to-CLS;
\}
...
profile by including pointers to its layer profiles. A complete image consists of:
\begin{compactenumerate}
...
\item Compression ratios \lrcomment{double check these two are correct}: \{
AIS-to-CIS;
FIS-to-CIS;
\}
