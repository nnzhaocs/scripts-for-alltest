...
\acomment{Could not locate the figures you are talking about. I see two figures layers size and image size.}
\acomment{Where is layers analysis when you say similarly? I wrote follwing para see for consistency.}
...
%Conatiner image size determines the runtime of container if it is not already cached locally. 
...
\begin{figure}[!t]
	\centering
	\subfigure[Image compression ratio]{\label{fig_reference_cnt_cdf}
		\includegraphics[width=0.215\textwidth]{graphs/image-compress-ratio-pdf.pdf}%
	}
	\subfigure[Layer compression ratio]{\label{fig_reference_cnt_pdf}
