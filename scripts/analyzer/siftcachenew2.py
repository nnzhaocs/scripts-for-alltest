...
rlmaplocation = "/home/nannan/keren/default.json" #"/home/nannan/dockerimages/docker-traces/data_centers/usr2repo2layer_map_with_size.json"
urlmaplocation = "/home/nannan/keren/default.json" #"/home/nannan/dockerimages/docker-traces/downloaded-traces/data_centers/dev-mon01_total_trace.json-repo2layersdic.json"
#max_repo = 0
def mean(items):
...
class siftcache:
    ...
    def __init__(self, threshold, cache_size):
...
#         self.pecentage
    ...
    def clear_buffer(self):
        ...
        self.URLmap = defaultdict(lambda: defaultdict(set))
    ...
    def prefetch_layers(self, request):
        ...
        #newlogic==
        try:
            ph = self.URLmap[client][repo]
        except KeyError:
            try:
                self.URLmap[client][repo] = set()
            except KeyError:
                self.URLmap[client] = {repo:set()}
        ...
        to_prefetch_excluding_putbuffer = list(set(prefetchable).difference(self.layer_buffer.keys()))
        #nannan
...
#             layer = request['getid']
    ...
    def evictions(self, size):#now_time):
        curr_size = sum([self.layer_buffer[layer]['size'] for layer in self.layer_buffer.keys()])
        ...
        '''if curr_size > self.cache_size:
            for layer in self.layer_buffer.keys():
                layer_time = self.layer_buffer[layer]['timestamp']
                #lru_lyr = self.layer_buffer.peek_last_item()
                timediff = now_time - layer_time
                if timediff > self.threshold:
                    curr_size-= self.layer_buffer[layer]['size']
                    del self.layer_buffer[layer]
                    self.total_evictions += 1
        self.layer_buffer_space_usage.append(curr_size)'''
        while curr_size+size > self.cache_size:
            #self.layer_buffer.clear()
            lru_lyr = self.layer_buffer.peek_last_item()
            if lru_lyr is None:
                break
            curr_size-= lru_lyr[1]['size']
            del self.layer_buffer[lru_lyr[0]]
            self.total_evictions += 1
        self.layer_buffer_space_usage.append(curr_size)
        ...
        '''for layer in self.layer_buffer.keys():
            layer_time = self.layer_buffer[layer]['timestamp']
            timediff = now_time - layer_time
            if timediff > self.threshold: #timediff.seconds > self.threshold:
...
#         self.sediments += len(set(self.prefetched_layers_buffer.keys()))
        to_prefetch_excluding_putbuffer = list(set(prefetchable).difference(self.layer_buffer.keys()))
        size = 0
        for layer in to_prefetch_excluding_putbuffer:
            size += self.RLmap[repo][layer]
        return size
...
def extract(data):
    ...
    for request in data:
        ...
        size = request['size']#request['http.response.written']
...
#         layer_or_manifest_id = parts[1] + '/' + parts[2] + '/' + str(size) # repo + layer id as layers unique identifier
        layer_or_manifest_id = request['dgst']#uri.rsplit('/', 1)[1]
        if t == 'm':
            size = 0
        requests.append({'timestamp': request['time'],#timestamp, 
                        'client': request['usr'],#['http.request.remoteaddr'], 
                        'method': method, #request['http.request.method'], 
                         'repo': request['repo'], #parts[1]+'/'+parts[2],
                        'type': t,
                         'size': size,
