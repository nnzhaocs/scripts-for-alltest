...
# with open(manifest_map_dir_filename) as f:
...
# with open(layer_json_map_dir_filename) as f:
# 	layer_json_map_dir = json.load(f)
# with open(layer_config_map_dir_filename) as f:
# 	layer_config_map_dir = json.load(f)
...
def write_json_datas(json_datas):
	...
	"""write image mapper"""
	if image_mappers:
		with open('image_mapper.json', 'w') as f:
			json.dump(image_mappers, f)
	if layer_analyzer_jobs:
		with open('layer_analyzer_jobs.json', 'w') as f:
			json.dump(layer_analyzer_jobs, f)
	if bad_manifests:
		with open('bad_manifests.json', 'w') as f:
