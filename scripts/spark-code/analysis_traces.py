...
CACHE_ANALYSIS_DIR = os.path.join(HDFS_DIR, 'cache_analysis')
...
usr2repo2layermap = spark.read.json(os.path.join(CACHE_ANALYSIS_DIR, 'usr2repo2layer_map.json'), multiLine=True)#.coalesce(40).write.save("/var/1g_big_json.parquet", format="parquet", mode='append')
