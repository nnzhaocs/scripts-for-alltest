...
def create_job_list():
    ...
    with open(list_50mb, 'w+') as f_out:
        tmp_dict = {key: val for key, val in d_ascending.items() if val <= 50*1024*1024}
        json.dump(tmp_dict, f_out)

    list_1gb = os.path.join(job_list_dir, 'list_less_1g.out')
    with open(list_1gb, 'w+') as f_out:
        tmp_dict = {key: val for key, val in d_ascending.items() if val > 50*1024*1024 and val <= 1024*1024*1024}
        json.dump(tmp_dict, f_out)

    list_2gb = os.path.join(job_list_dir, 'list_less_2g.out')
    with open(list_2gb, 'w+') as f_out:
        tmp_dict = {key: val for key, val in d_ascending.items() if val > 1024*1024*1024  and val <= 2*1024*1024*1024}
        json.dump(tmp_dict, f_out)

    list_b_2gb = os.path.join(job_list_dir, 'list_bigger_2g.out')
    with open(list_b_2gb, 'w+') as f_out:
        tmp_dict = {key: val for key, val in d_ascending.items() if val > 2*1024*1024*1024}
        json.dump(tmp_dict, f_out)
