...
P-servers fail, D-servers will serve the request.
...
The layer restoring process has two parts: slice constructor and layer constructor.
As shown in Figure~\ref{fig:restoring}, % shows an example of parallel layer restoring when a \texttt{pull} layer request failure happens.
first, 
layer constructor fetches the layer recipe from metadata database.
As shown in \emph{L1}'s layer recipe, 
...
Each file is written to a slice archive buffer asynchronously. % along with their corresponding headers saved in slice recipe.
