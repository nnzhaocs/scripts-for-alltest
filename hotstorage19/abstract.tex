...
The rapidly growing number of container images and the associated
performance and storage capacity demands are key obstacles to
sustaining and scaling Docker registries.  The challenge is the
inability of the extant layer-level content addressable mechanism to
effectively deduplicate the layers that are stored in a compressed format. 
...
heterogeneous cache comprising memory and flash storage.
Evaluation of the prefetching algorithm shows 
a high hit ratio of 96\%. Moreover, the two-tier heterogeneous cache is able to store 56\% more layers into the file cache.
