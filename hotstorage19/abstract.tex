...
pressure on the registries, which store and serve images. The large volume of Docker images stored in registries and the high file redundancy among the images are key obstacles
...
%registries.
%
Mitigating the high file redundancy in real-world images through deduplication is
a promising approach to drastically reduce the large storage requirements of the
registries.
%
...
compare to the current registry. 
For the \emph{highest performance mode},
\sysname can reduce the \texttt{GET} layer latency up to 14\%
