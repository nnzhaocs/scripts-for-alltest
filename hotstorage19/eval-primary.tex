...
We also evaluate \emph{BOLT} with 3-way replication on nine servers as our baseline. 
...
Our superfetch cache hit ratio is 0.91. This high hit ratio entails improvement to the primary servers' I/O latency. However, the network transfer time accounts for the largest portion of the \text{GET} layer latency for \emph{BOLT} and \emph{B mode 3}.

Next we compare \emph{B mode 0} with \emph{BOLT}.
Although \emph{B mode 0} saves half of the storage space by \emph{deduplicating} all layer replicas (see \S\ref{sec:inter-layer-deduplication}), it only increases the \texttt{GET} layer latency by at most 37\% due to the layer restoring overhead.
...
The \texttt{Fra} and \texttt{Lon} traces have 21\% and 15\%, respectively, of \texttt{GET} layer requests waiting for layer construction \Subil{We need a graph or something to show this waiting time because I don't see it from the existing figures}. 
...
%\Subil{I think there needs to be at least some mention of Dal and Syd?}
 In the \texttt{Dal} workload also, \emph{B mode 3} outperforms \emph{BOLT} for the same reasons. Across all the workloads, \emph{S mode} provides comparable latencies to \emph{BOLT} while providing savings on the storage side due to deduplication.
