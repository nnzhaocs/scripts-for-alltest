...
B mode 2 and B mode 1, the number of P-servers is set to 6 and 3, respectively.
%In addition, 
...
We also evaluate the \emph{Bolt} with 3-way replication on 9 servers as our baseline. 
...
Comparing \emph{Bolt} with \emph{B mode 3}, we see that \emph{B mode 3} improves the overall performance by using a \emph{superfetch cache}.
\emph{B mode 3} reduces the average \texttt{GET} layer latency by 13\% compared to \emph{Bolt} when the layer size is 20~MB.
Our superfetch cache hit ratio is 0.91. This high hit ratio entails a significant improvement to the primary servers' I/O latency. However, the network transfer time accounts for the largest portion of the \text{GET} layer latency for \emph{Bolt} and \emph{B mode 3}.

Next we compare \emph{B mode 0} with \emph{Bolt}.
Although \emph{B mode 0} saves half of the storage space by \emph{deduplicating} all layer replicas, it increases the \texttt{GET} layer latency by at most 37\% due to the layer restoring overhead.
...
%For \emph{B mode 1} which has a 3-node primary cluster, more \texttt{GET} layer requests are sent to the P-servers, which causes a lot of overhead.
...
This is because for \emph{S mode}, the hot layers have 3 replicas and are distributed across a 6-node primary cluster, which improves the overall performance.
%while the remaining cold layers only have a single layer replica on the primary cluster. 
...
As shown in Figure~\ref{fig:eval-total-traces},
\emph{B-mode 0} increases the \texttt{GET} layer latency by 39\% and 80\% on average
for workloads \texttt{Fra} and \texttt{Lon}
because of layer restoring overhead.
\texttt{Fra} and \texttt{Lon} has 
lower preconstruct cache hit ratios of 0.72 and 0.8 with 
21\% and 15\% of \texttt{GET} layer requests waiting
for layer construction because of small duration between 
\texttt{GET} manifest requests and the subsequent \texttt{GET} layer requests.
Note that the \emph{superfetch cache} hit ratio is
not affected by the duration \texttt{GET} manifest requests and the subsequent \texttt{GET} layer requests.
Therefore, \emph{B-mode 3}
outperforms \emph{Blot} for both \texttt{Fra} and \texttt{Lon}.
