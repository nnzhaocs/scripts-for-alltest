...
Note that B-mode 3 does not deduplicate layer replicas.  While B-mode 0
\emph{deduplicates} all layer replicas. 
%\sysname is evaluated by configuring different deduplication modes.
Accordingly, B-mode 3 doesn't have D-servers while B-mode 0 doesn't have
P-servers.
%
For the remaining two B-modes -- B-mode 2 and B-mode 1, the number of P-servers
is set to six and three, respectively. For S-mode, the number of P-servers is
set to six.
%In addition, 
...
To generate workalod, we use 100 clients replaying the traces to P-servers and/or D-servers
...
modes and \emph{BOLT} by replaying the \texttt{Dal} workload five times, each
time with a different layer size (from 5~MB to 25~MB).
%
Note that for \emph{B-mode 0}, the \texttt{GET} layer latency is the
...
Overall, the average \texttt{GET} layer latency increases with layer size for
all the schemes.
%
...
%
Moreover, when the
layer size is 5~MB, the average \texttt{GET} layer latency across all the
different schemes is around 0.2~s with minimal variation.
%
For example, the
...
layer latency by at most 37\% due to the layer restoring overhead.
...
For the remaining basic modes: \emph{B-mode 2} and \emph{B-mode 1}, we see that
...
to preserve the temporal properties and use the layer size of $\sim25$~MB
...
Figure~\ref{fig:eval-total-traces} shows that due to layer restoration overhead
...
requests waiting for layer construction \Subil{We need a graph or something to
show this waiting time because I don't see it from the existing figures}.
%
