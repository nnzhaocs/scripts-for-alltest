...
performs in-line ransparent data deduplication and resides between the client's application and the hosting cloud storage.
...
Moreover for restoring a layer, all the layer's files should be fetched from wherever they reside, as they are likely to be scattered across multiple servers, 
and then compressed together, which causes a considerable overhead on pull layer requests performance.

To quantify how much performance overhead is caused by using deduplication on the Docker registry,
we setup five registry instances, each with a local file system as their backend storage system,
and implemented file-level deduplication with decompression and compression operations.
We replayed the IBM registry workload \texttt{dal}~\cite{dockerworkload} 
%by sending requests randomly 
randomly to our five registries and presented average latency 
as shown in Figure~\ref{fig:avg_latency_dedup_nodedup}.
Note that since 
%the IBM registry workload 
\texttt{dal} does not contain real layers,
we extract the layer digest from each request and match it with a layer randomly selected from our Docker Hub dataset.

Without deduplication,
the average latency for requesting a layer is around~2\,s for layers with sizes less than~50\,MB.
...
because it is highly likely they will be requested again.
However, it is the opposite for the layer access pattern: if a layer is pulled by a user,
then this layer will not be pulled by this user in the future
since this layer is locally available to the user.
Ali Anwar et al. proposed a push-pull prefetching algorithm to improve the cache hit ratio~\cite{dockerworkload}.
Based on our observations, 
%we found the duration between two subsequent layer requests is very long,
we found that the inter-access time for layer pulls is very long.
Thus, if we cache a layer, we might need to wait for hours to get a hit on that layer.

\input{fig-skewness-reusetime}

\input{observation-traces}
