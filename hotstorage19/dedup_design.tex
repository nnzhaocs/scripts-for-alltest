...
predict because some layers are only accessed infrequently.
From \arb{???}, we have observed that about half of the cached layers are not
accessed again for at least $1.3$~hours. This means that if we
cache a layer, we may need to wait a long time before we observe a hit on that layer (as discussed in Section~\cref{sec:background}).  
This is mainly 
...
them in to a tar file \arb{that can service the ??? request}. 
...
%\subsection{Design}
To address these issues, we propose a new registry design. The key feature of our design is a user-behavior-based cache that helps mitigate the performance degradation due to the 
backend dedup storage system (Figure~\ref{fig:sys-overview}).  Based
on our observation in~\cref{sec:background}, a user's active time is predictable. 
Thus, we leverage users' behavior, \ie
when a user is most likely to be active, to drive layer evictions from the cache.

Considering that layer sizes are typically about several megabytes~\cite{dockerworkload}, a small main memory cache will be unable to accommodate
active users' layers (for any reasonable/practical number of users). To address this issue, we 
create separate caches for layers and \emph{unique} files, called {\em layer buffer} and {\em file cache}, respectively. Both caches comprise both
main memory and flash memory.
\arb{are main memory for one type and flash for the other type, or both for both types. I assumed both types of memory are used, and there are two caches. check previous sentence for correctness.}
Note that, layers are  compressed tarballs, and 
%sent by users
 \emph{unique} files are uncompressed files from which duplicates have been removed. 
...
scattered across multiple servers. We define all the per-server files belonging to a layer as a {\em slice}. A server stores slices for many layers, and a layer is composed of slices stored on multiple servers.
