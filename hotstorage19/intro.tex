...
%
For example, Docker Hub~\cite{docker-hub}---a popular public container registry---stores more than~2 million public repositories. These repositories have grown at the rate of about $1$ million annually. A similar rate of growth  would require provisioning an additional 2.5~TB of storage per week (and is expected to increase).
%
This puts intense pressure on Docker registry storage infrastructure, and the problem has so far remained largely unexplored.
%
To mitigate this problem, in this paper, we propose a new Docker registry architecture,~\sysname,
that integrates caching and deduplication with Docker registries to help reduce the storage requirements while keeping the performance overhead to a minimum.
...
Our analysis of around~47\,TB (167\,TB uncompressed) of Docker images downloaded from Docker Hub (containing over~5 billion files) reveals
that only~3\% of the files are unique; the remaining are redundant copies. 
...
Many cloud storage systems already implement deduplication to effectively eliminate redundant data.
...
because of the unique dataset in the Docker registry: compressed layers. 
In contrast to uncompressed files, compressed files have a lower deduplication ratio. \sysname~addresses this by decompressing layers before performing deduplication.
...
multiple servers. 
To restore a layer,
the layer's files should be first fetched from multiple servers, then compressed as a layer
...
Leveraging these registries as a cache to temporarily store popular layers and
improve pull latency seem intuitive.
...
%%%%
First, we observe that layer sizes vary from a few MB to several GB and that a majority of layers sizes are around several MB.
...
comprising a layer buffer and a file cache to hold  popular layers:
