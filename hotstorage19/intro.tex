...
%Coupled with microservices, the container ecosystem has changed the way resources are deployed into the cloud.
%Kubernetes engine, cloud storage.
%Docker registry, cloud storage, gzip compressed tarfiles
%Redundant data stored among compressed tarfiles
%Cannot directly apply blk or file-level dedup, and restoring overhead
%Restoring overhead slows down normal layer access.

% 1. Containerization and microservices grow more than faster.
 %Amazon, Netflix, and eBay, container as a service
 %(memory and cpu resource limits), (pid, networking namespace, mount namespace, etc.)
The rise of \emph{containerization} has been a remarkable advancement in 
cloud environment and architectures,
allowing applications to build, deploy and manage faster and easier.
At its core, containers 
refactor a traditional, monolithic architectures into a flexible, portable microservices platform 
to organize and distribute applications. 

As a popular and successful example of container advances,
...
Moreover, 
Docker provides a systematic way to 
automate the faster deployment of applications inside portable containers.
Docker containers managed by Kubernetes orchestration system
are clustered together to provide high available, scalable, and secure services.
...
A Docker image may include OS fundamentals, system libraries, dependencies, or executable codes. 
When building images with Docker, 
each action taken (that is, 
command executed, such as \texttt{apt-get install}) forms a new pluggable \emph{layer} on top of the previous one. 
Docker leverages a COW (copy-on-write) file system 
which allows Docker to instantiate containers quickly. 
COW mechanism also improves Docker storage system efficiency. 
%by minimizes I/O and the size of each of the subsequent layers.
%because it leverages 
%the pointers to the existing files. 
%Copy-on-write file system also provides layering of containers, thus you can create a 
%base container and then have another container which is based on the base container. 
%Commands can be executed manually or automatically using Dockerfiles.

Docker images are stored as a set of shareable and content addressable \emph{compressed layer tarballs} 
and content distributed by Docker registry like Docker Hub.
Docker registry delegates storage to different drivers using content addressable store-like API.
A layer is uniquely identified by a collision-resistant hash of its content.
No duplicate layers are stored in registry.
Drivers include methods of reading, writing, and deleting content of 
the \emph{backend storage systems}
such as \emph{local file system} or \emph{Cloud storage systems} (e.g., S3, Azure, Swift, OSS, GCS).

Cloud storage systems usually use \emph{replication} to maintain high availability and tolerant server failure.
Each layer has multiple \emph{layer replicas}.
As container market grows rapidly, 
more images and layers are built, \emph{replicated}, and stored in Docker registry.
This puts intense pressure on the availability and scalability of Docker registry's backend storage infrastructure 
because scaling-out involves data migration between existing storage servers and new storage servers, 
even among existing storage servers for load balancing,
which largely hurts performance.

Many cloud storage systems utilize deduplication to remove duplicate data and better storage allocation, resulting in
a significant cost savings on hardware upgrades and scale-out effectively.
...
multiple servers. 
To restore a layer,
the layer's files would need to be first fetched from multiple servers, 
then compressed as a layer (to preserve transparency and avoid client-side API modifications)
and sent back to the client.
The extra network, I/O, and computation will slow down the response time for retrieving (\texttt{pulling}) an image.

In this paper, we propose \sysname, a fast and lightweight Docker registry.

%Docker image dataset is unique while treated equally as objects, files, or blocks.
%%%%%%%%%%%%%%%%%%% old intro %%%%%%%%%%%%%%%%%%%%%%%%%%%
