...
Containers are running instances of \emph{images} which contains complete runtime environments for an application.
...
Many container registries use remote cloud storage like S3~\cite{xxx}, 
Microsoft Azure~\cite{xxx}, OpenStack Swift~\cite{xxx}, and Aliyun OSS~\cite{xxx} as their backend storage systems as shown in Figure~\ref{fig:without-dedup}.
For example, Google container Registry~\cite{xxxx} stores images on cloud storage while
...
For example,
For example, Google cloud and AWS, 
performs in-line data deduplication transparently.\NZ{use a new example}
However, existing deduplication techniques cannot be directly applied to the image storage system 
because of unique workload in Docker registry: compressed layers. 
In contrast to uncompressed files, compressed files have a lower deduplication ratio. 
So the layers should be decompressed before performing deduplication.
...
These extra network, I/O, and computation latency will slow down image pull response time.
As shown in Figure~\ref{xxx},
without deduplication,
the average latency for requesting a layer is around 2\,~s for layer size less than 50MB.
While the latency increased to 12\,s when deduplication is implemented in backend storage system.
Docker registry performance drop down dramatically for bigger layers.
We see that the average latency for requesting layers bigger than 50MB and smaller than 1GB
is around 128\,s while it worsen when dedupliation is implemented, the average latency increased to around
800\,s.
...
that integrates cache, deduplication, and Docker registries to address the aforementioned obstacles.
%low restore latency %adaptive%
% deduplication framework for Docker registries. 
Large container registry service provider like Google and IBM use regional private repositories across the world to facilitate a fast and high-availability service~\cite{xxx}.  
...
Therefore, registry can be used as a cache to temporarily store popular layers
to reduce remote cloud storage access latency.
Although cache has been studied as web cache or proxy cache, 
this paper addresses the unique challenges in integrating cache with deduplication in Docker registry.

First, layer size varies from few MB to several GB.
We observed that majority of layers are around several MB.
Small main memory cache cannot accommodate many popular layers.
Second, 
We observed that although layer accesses are heavily skewed, the duration between two subsequent layer access requests is too long, 
which means if we cache a layer, we might need to wait hours to get a hit on that layer.
To address these issues,~\sysname~embodies a two tier cache architecture 
comprising a layer buffer and a file cache to hold as many as more popular layers:
Layer buffer stores popular layers in main memory while file cache stores 
the \emph{non-deduped} unique files for the victim layers that are evicted from layer buffer on flash-based storage system to improve cache space utilization.
Moreover, we proposed a user-based cache algorithm to improve cache hit ratio 
...
\sysname's hit ratio~can achieve 20\% higher than traditional LRU. 
...
We present ~\sysname~design in Section~\ref{sec:\sysname} and our preliminary evaluation in Section~\ref{sec:Evaluation}. 
lastly, we describe related work in Section~\ref{sec:related},
