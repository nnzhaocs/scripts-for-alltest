\subsection{The mix of deduplication and replication of layers}
...
%
%
%\LR{There should always be some text after a heading and before the
%next heading.}
...
%Then we outline how \sysname performs low-latency layer restoring.
Moreover,
...
% replicas will be temporally saved into layer stage areas as shown in Figure~\ref{fig:sift}.
 \sysname initiates an \emph{off-line lightweight layer deduplication process} on a D-server 
...
%##### need to record all files in the tar archive, not just unique files' header but also the deduplicated files'headers. otherwise, how do you restore a complete tar achieve?????
After that,
...
%\textbf{file status}
%
%\LR{Is the file status the same as the file header?}
%
