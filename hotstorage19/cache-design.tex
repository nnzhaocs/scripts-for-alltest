...
\sysname uses two maps for recording image-to-layer architecture 
and user's layer access information:
...
\sysname first calculates a set of image layers that haven't been \texttt{pulled} by the user \emph{r.client} 
by 
calculating the difference $S_{\Delta}$ between the image's layer set and user accessed layer set:
...
\sysname first gets a subset of layers from the image that have already been \texttt{pulled} by the user 
by calculating the 
intersection $S_{\bigcap}$ between the image's layer set and the user accessed layer set as follows:
$S_{\bigcap} = \emph{RLmap[r.img]} \bigcap \emph{ULmap[r.client]} $.
Next, 
\sysname compares the client repull probability 
$\gamma[r.client]$ with a predefined client repull threshold $\varepsilon_{c}$.
...
\paragraph{Tiered storage architecture} 
As mentioned earlier,
\sysname uses two-tier storage architecture: primary cluster and deduplication cluster.
Now with the adding of superfetch cache and preconstruct cache,
The two-tier storage architecture becomes
multiple-level storage architecture as shown in Figure~\ref{fig:tieredstore}.

First,
Upon a \texttt{GET} layer request,
\sysname first searches the superfetch caches in primary cluster.
If the layer presents in the cache, 
the request will be served from cache.
Otherwise, the request will be served from the layer stores in primary cluster.
...
%
If a \texttt{GET} layer request misses on primary cluster due to P-server failures,
the request will be forward to deduplication cluster.
First, \sysname searches the layer stage areas on deduplication cluster.
If not found, \sysname searches the preconstruct caches.
If again not found, \sysname will rebuild a layer to serve the request.

Note that both superfetch cache and preconstruct cache are write-through caches.
When the layers are evicted, 
it will simply be discard since there will be no updates to layers.
Layer replicas temporally stored layer stage area
will be first evicted to preconstruct cache after the layer is deduplicated
and then discard from preconstruct cache after it's replaced.
For preconstruct cache,
it's possible when a \texttt{pull} layer request comes 
