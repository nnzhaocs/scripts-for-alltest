\subsection{Caching and Preconstructing Layers}
...
two data structures:
%
%Therefore, it uses two maps to record the image-to-layer information
%and user layer accesses:
...
\begin{figure}[t]
	\centering
	\centering
	\includegraphics[width=0.4\textwidth]{graphs/sift-cache-bigger.pdf}
	\caption{Tiered storage architecture.}
	\label{fig:tieredstore}
\end{figure} 

\paragraph{Cache handling in tiered storage} %As mentioned earlier, \sysname uses two-tier storage architecture: primary cluster
...
%\Subil{We say 5-tier here but 2-tier in the figure. Must change one or the other for consistency along with any other mentions of 2-tier/5-tier in the paper}.
%\NZ{can we say 2-tier with 5-level?} \Subil{changed to 5 level/2 tier}
...
Both the superfetch and the preconstruct caches are write-through caches.
...
%ARC is best suited for our purposes because it is known to handle varying workloads
%without prior tuning. %The real-life Docker registry workloads have varied access 
%patterns between them and so a caching solution that would work effectively 
%without needing to be fine tuned for each observed workload was necessary. 
...
%\Subil{addressed, but may need editing}
%\NZ{because ARC adapts between LRU and LFU. 
%Layer accesses are skewed according to anwar's paper so LRU will work but
%in case that layer reuse time is higher,
%so LFU is also needed.}
%\Subil{incorporated the reasons into text}
