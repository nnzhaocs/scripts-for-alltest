...
%	\let
	\scriptsize
	\begin{tabular}{| p{1.5cm} | p{1.5cm} | p{1.5cm} | p{1.8cm} |}  %[-1cm]
		\cline{1-4}
		\multirow{2}{2cm}{Dedup schemes} &  Dedup layers as it is  & \multicolumn{2}{ p{3.8cm} |}{ Dedup layers after decompressing them first}   \\ \cline{2-4}
		             & Dedup ratio  &    Dedup ratio       &      Restore latency     \\ \cline{1-4}
	VDO	          &   1                &     4 x      &       57 x      \\ \cline{1-4}
	Btrfs          &  1                  &      2.1 x     &      51 x       \\ \cline{1-4}
	zfs             &   1                 &     2.3 x      &      50 x       \\ \cline{1-4}
	Jdupes       &   1                &     2 x      &        36 x     \\ \cline{1-4}
    Ceph	      &   1                &           &        98 x     \\ \cline{1-4}
	\end{tabular}
...
%	%\tiny 
%	\scriptsize
%	\caption{Dedup ratio and layer restoring latency (i.e., layer pulling latency).}
%	\resizebox{\columnwidth}{!}{%
%		\begin{tabular}{| c | c | c | c | c | c | c | c |} 
%			\hline
%			\textbf{Storage systems/tools with deduplication support}    &   \textbf{Dedup layers as it is} & \textbf{Dedup layers after decompressing them first} \\ 
%			& \textbf{\#PUT L} & \textbf{\#PUT M} & \textbf{\#Uniq L} & \textbf{\#Uniq M} 
%			\hline\hline
