...
%up to 0.96.
Figure~\ref{fig:hitratio}, shows the hit ratios. We observe a significant increase to the hit ratio from $74$\% to $95$\% as the duration threshold grows from $1$ to $10$ minutes and more layers are prefetched.
...
confirming that it takes a longe time for a newly \emph{pushed} layer to be pulled.
...
there is no benefit to \emph{deduping} layers 
because the deduplication ratio is very low.
However, when the dataset's size is $3$~TB, we can store $56$\% more \emph{deduped} layers' in file cache.
