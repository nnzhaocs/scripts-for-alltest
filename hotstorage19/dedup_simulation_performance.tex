...
We simulate the user access history-based cache to evaluate the hit ratio 
by replaying the \texttt{dal} workload~\cite{dockerworkload}.
%as shown in Figure~\ref{fig:hitratio}.
%Our algorithm exhibits an enhanced cache performance, with a high hit ratio
%up to 0.96.
Figure~\ref{fig:hitratio}, shows the hit ratios. We observe a significant increase to the hit ratio from $0.74$ to $0.95$ as the duration threshold grows from $1$ to $10$ minutes because
 more prefetched layers can be kept in cache for a longer time.
The hit ratio stablizes at $0.96$ as the duration threshold increases from $15$ to $20$ minutes.
%Therefore,
...
The layers causing the $4$\% miss rate, are due to 
\emph{re-pulls} by the same user.
...
We next compare the prefetching hit ratio with the layer buffer hit ratio.
We see, across different duration thresholds, 
prefetching hit ratio is much higher than the layer buffer's hit ratio,
confirming that it takes a longer time for a newly \emph{pushed} layer to be pulled.
...
%
In Figure~\ref{fig:cacheefficiency}, the x-axis values correspond to the sizes of $4$ random samples drawn from the whole dataset and the size of the dataset in terms of layer count and capacity.
...
However, when the dataset's size is $3$~TB, we can store $56$\% more \emph{deduped} layers' unique files in file cache.
...
This verifies the high benefit of the file cache when the file cache size is large.
We should carefully select the size of the file cache to save significant space.
