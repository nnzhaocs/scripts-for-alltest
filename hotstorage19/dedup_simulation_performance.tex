...
From Figure~\ref{fig:hitratio} we also can see that 74\% of users can finish their \texttt{pull} layer request
within a minute and 
around 89\% of users can finished their \texttt{pull} layer request with less than 5 min. 
Moreover, compare hit ratio for prefetched layers (i.e., prefetch hit ratio) and layer buffer hit ratio,
we see that in a short time period, 
our prefetched layers have a much high hit ratio than layer buffer across different duration threshold,
which means that it takes longer time for a newly \emph{pushed} layer to be pulled.
...
%
As shown in Figure~\ref{fig:cacheefficiency}The x-axis values correspond to the sizes of 4 random samples drawn from the whole dataset and the size of the dataset in terms of layer count and capacity.
