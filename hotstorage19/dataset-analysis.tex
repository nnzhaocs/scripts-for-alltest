...
image, which includes references to all the layers in the image.
...
%
However, the client might or might not follow up with a layer \texttt{GET} requests.
%
Further, there might be not enough time to preconstruct the layer
between the manifest and layer \texttt{GET} requests.
...
layer preconstruction might not be finished \emph{in time}. However, even in that case layer preconstruction can still be beneficial because
