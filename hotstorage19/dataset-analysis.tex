...
registry~(\S\ref{sec:predictable-user-access}) and preconstructing layers before they are requested~(\S\ref{sec:layer-preconstruction}) to reduce the deduplication
...
\NZ{can we just say 3\% of unique files without mention the space savings?} \Subil{Though I agree that mentioning the 2x space savings doesn't really fit well here, we need to make sure this 2x space saving is mentioned somewhere because we say in section 6.3 that full deduplication saves half the storage space. so 2x space saving should be talked about somewhere before then, if not here}
%
The large amount of duplicate files
...
%
Therefore, we analyze the duration between a \texttt{GET} manifest request and the subsequent
...
In the case of small duration between a \texttt{GET} manifest request and its subsequent \texttt{GET} layer requests,
layer preconstruction probably cannot be finished \emph{in time}. However, layer preconstruction can still provide a lot of benefit because
the layer construction starts before the arrival of a \texttt{GET} layer request,
which can largely reduce the layer construction overhead in the \texttt{GET} layer request latency. In summary, it is feasible to start building a layer for the user before
