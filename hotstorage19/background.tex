...
Docker container is a running instance of an \emph{image} which wrap up an application and its runtime dependencies.
...
it delegates storage to drivers which interact with either a local file system or a remote cloud storage like S3~\cite{xxxx}, Microsoft Azure~\cite{xxx}, OpenStack Swift~\cite{xxxx}, and Aliyun OSS~\cite{xxx}. 
For example, Google Container Registry~\cite{GoogleContainerRegistry} use cloud storage as their backend Docker image storage systems.
Users \texttt{push} and \texttt{pull} Docker images to and from their repositories stored on cloud storage. 
