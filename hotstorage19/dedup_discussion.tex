...
1. Seamlessly integrate backend dedup process with prefetch cache algorithm

In the future work, we will analyze how long it will take to fetch a layer from
backend storage system.
If the latency of prefetching a layer is longer,
the incoming pull layer requests will not be served immediate.
Next, we will analyze the cost-benefit of 
prefetching a layer.
Prefetching a layer from cold repositories can serve an immediate pull request,
but this layer will be deleted from cache after a short period of time 
because it won't be accessed again by other users 
but prefetching this will consume a lot of resources, 
for example, CPU, network bandwidth, I/Os. 
Moreover, this layer staying in cache might trigger other layers from hot repositories to be evicted.
Another question is how much is the performance again by using file cache.
Although file cache can increase space efficiency, once layer request is hit in layer cache,
we need to do parallel compression and it also cause a latency.
...
2. Another angel to reduce redudant data in Docker registry: Docker client-side deduplication

Client-side deduplication affects the container runtime. 
This is because performing inline file-level deduplication on the Docker client side requires intensive file 
fingerprint calculations, and file fingerprint lookup, 
...
remove duplicate files in their local file systems without performance overhead.
