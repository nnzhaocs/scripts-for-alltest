...
Then, one layer replica (say, at server) is \emph{deduplicated} into unique files, denoted as \emph{primary file replicas}.
After that, each unique file is replicated and stored on two different servers: \emph{B} and \emph{C} (denoted as primary and backup file replicas respectively).
In the end, 
two layer replicas are discarded from server: \emph{B} and \emph{C}.
...
In this case, \sysname has the same level of redundancy as the original registry cluster. 
