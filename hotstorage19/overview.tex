...
As shown, upon a \texttt{push} layer request, 
registry \emph{A} in original registry cluster stores the layer locally, denoted as \emph{primary layer replica}.
Meanwhile \emph{A} replicates the layer to other two servers: \emph{B} and \emph{C}, denoted as \emph{backup layer replicas}.
The \texttt{pull} layer request can be served by any of the three layer replicas.
Similar to original registry cluster, 
\sysname first stores three layer replicas for newly \texttt{pushed} layers on three different servers: \emph{A}, \emph{B}, and \emph{C}.
Then, one layer replica is \emph{deduplicated} into unique files, denoted as \emph{primary file replicas}.
After that, each unique file is replicated and stored on two different servers: \emph{B} and \emph{C} (denoted as backup file replica).
In the end, 
two layer replicas are discard from server: \emph{B} and \emph{C}.
\texttt{pull} layer request is sent to registry \emph{A} which stores the primary layer replica.
When registry \emph{A} crashes,
Registry \emph{B} or \emph{C} can restore a layer for incoming \texttt{pull} layer request.
In this case, \sysname has the same level of redundancy. 
...
While D-server deduplicates backup layer replicas and  stores file replicas.
...
The \emph{Layer stage area} temporarily stores newly added backup layer replicas as shown in Figure~\ref{fig:sift-original}.
...
Meanwhile, each unique file is replicated to the peer servers to maintain the same level of redundancy.
...
\emph{Layer diskcache} on a D-server is an on-disk layer cache which caches popular layers for later layer accesses
to save restoring latency.
%As shown in Figure~\ref{fig:cache}, 
When a P-server crashes,
the \texttt{pull} layer request will be served from a D-server.
If the \texttt{pull} layer request hits in D-server's layer stage area,
...
based on its associated \textbf{layer recipe}(see section~\ref{sec:restore-desgin}). 
%After that, the restored layer is stored in the layer diskcache as shown in Figure~\ref{fig:cache}.
\sysname also preconstructs layers and caches them in the layer diskcache to improve the cache hit ratio (see section~\ref{sec:cache-design}). 
The layer diskcache is a write-through cache; when layers are evicted from the diskcache, they are simply discarded.

Deduplication related metadata such as layer recipe, \textbf{slice recipe}, \textbf{layer index}, and \textbf{file index} is kept in a \emph{distributed database} (Figure~\ref{fig:sift}) for 
reliability, consistency, and fast accesses. 
Slice recipe and layer recipe are used to restore slices and rebuild layers.
Layer index  
and file index records \emph{content fingerprints} that are uniquely mapped to the associated physical layers and files respectively. %psychical file based on
