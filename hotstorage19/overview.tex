...
\NZ{We use 
	the superfetch cache on registry side instead of on backend storage system to save some workload overhead to backend storage system.}
...
%
\LR{We should also explain here the File and Layer indices shown in the Figure.}
\NZ{
Besides, 
\sysname maintains a layer index to make sure an identification layer is not already stored.
To eliminate duplicate files from layer,
\sysname maintains a file index to keep record the unique files stored in the system.}
...
\LR{Talk about, how effective this approach is, if the gap between get manifset and
get layer is short. Can we still load/preconstruct layers fast enough to be served
from the cache once the get layer request is received?}
\NZ{The effective of the approach depends on 
the duration between a \texttt{GET} manifest request and its subsequent \texttt{GET} layer requests.
As shown in Figure~\ref{fig:intervals}, majority of intervals are greater than 1 s.
)
%
We shown the effect of duration in~\S\ref{sec:eval-dedup}}.
