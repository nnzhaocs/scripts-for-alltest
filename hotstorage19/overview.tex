...
each registry server's local storage system is
divided into three parts: layer stage area, layer diskcache, and file store.

The \emph{Layer stage area} temporarily stores recently \texttt{PUT} layers by clients as shown in Figure~\ref{fig:cache}.
...
The layer is then deleted from layer stage area.
To improve layer restoring performance, 
a layer is evenly divided into several \textbf{slices} and distributed across different registry servers so that
the layer can be restored in parallel.
  
\emph{Layer diskcache} is an on-disk layer cache which caches popular layers for later layer accesses.
As shown in Figure~\ref{fig:cache}, 
if a \texttt{GET} layer request hits in layer stage area,
the layer will be moved to \emph{layer diskcache}.
%
Upon a \texttt{GET} layer request miss, \sysname will rebuild the layer from the file store
...
\sysname also preconstructs layers and caches them in the layer diskcache to improve the cache hit ratio. 
The layer diskcache is a write-through cache; when layers are evicted from the diskcache, they are simply discarded.

Deduplication related metadata such as layer recipe, \textbf{slice recipe}, and \textbf{file index} is kept in a \emph{distributed database} (Figure~\ref{fig:architecture}) for 
reliability, consistency, and fast accesses. 
Slice recipe and layer recipe are used to restore slices and rebuild layers.
The file index records \emph{file fingerprints} that are uniquely mapped to the associated physical files. %psychical file based on
...
In the following, we describe how \sysname performs layer deduplication and layer restoring.
