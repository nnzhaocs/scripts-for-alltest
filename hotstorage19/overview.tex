...
Figure~\ref{fig:sift-original} shows a simple deduplication mode of \sysname's registry cluster compared with original registry cluster.
...
Meanwhile \emph{A} replicates the layer to the other two servers: \emph{B} and \emph{C}. The replicated layers are denoted as \emph{backup layer replicas}.
The subsequent \texttt{pull} layer requests can be served any of the three layer replicas.%depending on the registry server's availability?
Similar to the original registry cluster, 
\sysname first stores three layer replicas for a newly \texttt{pushed} layers on three different servers: \emph{A}, \emph{B}, and \emph{C}. \emph{A} stores the primary layer replica, \emph{B} and \emph{C} store the backup layer replicas. %server roles/group discussed later, a forward reference maybe?
Then, one of the backup layer replicas (say, at server B) is \emph{deduplicated} into unique files, denoted as \emph{primary file replica}. \HA{It is not clear in the previous sentence that primary file replica is a collection of all the unique files constituting the layer or a file (one file)? Also, we should state why only one server is deduplicating i.e. the performance effect}
...
A subsequent \texttt{pull} layer request for that same layer is first sent to registry \emph{A} because it stores the primary layer replica.
If registry \emph{A} crashes,
either registry \emph{B} or \emph{C} can reconstruct and serve the layer. %for the incoming \texttt{pull} layer request.
%As a result, 
\sysname thus provides the same level of redundancy as the original registry cluster. 
\sysname provides different \emph{deduplication modes} by deduplicating a different number of layer replicas
...
While D-servers deduplicate the backup layer replicas at the file level, store the unique files and replicate them.
...
be explained here.}
%
The \emph{Layer stage area} temporarily stores newly added layer replicas as shown in Figure~\ref{fig:sift-original}.
After \texttt{deduplicating} a layer replica, 
the resulting unique files are stored as primary file replicas in a content 
...
a \HA{is it a deduplicated layer replica?} layer replica is evenly divided into several \textbf{slices} and distributed across different registry servers so that
the layer can be restored in parallel (see section~\ref{sec:dedup-desgin}).
  
The \emph{Preconstruct cache} on a D-server is an on-disk layer cache which hosts preconstructed popular layers for later layer accesses to mitigate layer restoring latency (see section~\ref{sec:cache-design}).
...
Upon a \texttt{pull} layer request miss, the D-server will rebuild the requested layer from files in the file store
