...
A \emph{basic deduplication mode} (B mode) $n$ defines that \sysname should only
...
\NZ{R is replication level used by distributed storage systems. it's different with our mode n. n <= R.}
%
...
\NZ{yes}
\VT{Why keeping more than 1 non-deduped layers helps?} \Subil{Because we're distributing traffic among the replicas. The replicas are stored on different servers. More replicas means faster GET responses}
...
\texttt{pull} request.
\Subil{Nannan, please check to see if the above rewording is an acurate representation of the deduplication and replication process. I think this addresses Vasily's below comment}
%
...
and how it works?}
\NZ{removed original system. the original systems are the current used backend distributed storage systems.}
