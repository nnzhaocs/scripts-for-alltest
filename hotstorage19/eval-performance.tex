...
\begin{figure*}[t]
	...
	\begin{minipage}{0.3\textwidth}
		...
		\caption{GET layer latency\todo{add grid in background to match the other figures}
		\LR{Change `deduplication' to `\sysname' in the legend}}% across different schemes.}
	...
	\caption{\texttt{GET} manifest/layer IAT \LR{Change xlabel to ``GET manifest/layer inter-arrival time''.}
	\LR{We should also reorder the x-axis going from None (i.e. 0) to 1, not the other way round.}}
...
We match requests with layers with similar size.
...
\begin{figure*}[t]
	...
	\caption{Restoring latency breakdown. \Subil{maybe change 'fetch and merge slices' to
	'layer construction duration' to keep consistency with the text}}
...
%
\LR{Can we just call it \sysname instead of \sysname-dedup?}
\NZ{yes.}
%
...
\NZ{pgzip is for layer restoring. We use standard gzip for deduplication.}  \Subil{Since we are only using pgzip for compression for layer restoring for pulls and standard gzip for decompression during a push, we should remove the implication that pgzip is used for both compression and decompression at the end of section 5.2}
%
