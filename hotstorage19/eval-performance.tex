...
\subsection{Restoring performance of deduplication cluster }
\label{sec:eval-dedup}

%We map the traces with two different layer groups: 
%small layers (layer size $\leq$ 50 MB)

\paragraph{Restoring performance overhead and the benefit of preconstruct cache}:
We first measure the overhead of deduplication on layer restoring latency 
over LocalFS
by running \sysname on a single server.
...
We replay 1500 requests from \texttt{Dal}
and match them with different layer sizes.

%\paragraph{Cache}
As shown in Figure~\ref{xxx},
the average \texttt{GET} layer request latency increase as layer size.
...
Restoring a layer put 150\% overhead on \texttt{GET} layer latency.
By adding a normal LRU cache,
the average \texttt{GET} layer latency decreases by 39\%.
With our preconstruct cache,
the average \texttt{text} layer latency descreases by 24\% over a normal LRU cache.
Compare with localFS, our \sysname with preconstruct cache only adds 19\% of overhead. 

Next, we descrease the duration between \texttt{GET} manifest request and its subsequent \texttt{GET} layer requests.


Figure~\ref{xxx} shows the cache hit ratio of a normal LRU and our preconstruct cache.

\paragraph{Cluster scale impact}
We increase the number of nodes in our sift deduplication cluster
and test the layer restoring performance.
