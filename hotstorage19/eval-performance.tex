...
\paragraph{Restore performance overhead and preconstruct cache benefit} 
We first measure the overhead of deduplication on layer restoring latency over LocalFS by running \sysname on a single server.
We launch a client on another server and \texttt{pull} three layers in parallel.
We evaluate three configurations for \sysname; \sysname as a registry without deduplication, as a deduplication registry, as a deduplication registry with ARC cache~\cite{xxx}, and as a deduplication registry with a preconstruct cache.
We set the cache size to 20\% of ingress data.
...
Next, we decrease the duration between a \texttt{GET} manifest request and its subsequent \texttt{GET} layer requests.
